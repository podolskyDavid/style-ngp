...
import argparse
from util import switch_coord_system, reverse_transform
...
def compute_add_loss(mesh, start_pose, gt_pose, pred_pose):
    ...
    # min_distances = np.zeros(pred_transformed_vertices.shape[0])
    #
    # # For each point in pred_transformed_vertices, find the closest point in gt_transformed_vertices
    # for i, point in enumerate(pred_transformed_vertices):
    #     distances = np.linalg.norm(gt_transformed_vertices - point, axis=1)
    #     min_distances[i] = np.min(distances)
    #
    # # Compute ADD-S
    # add_s = np.mean(min_distances)
...
def visualize(mesh, start_pose, gt_pose, pred_pose):
    ...
    # Add the cameras
    points = np.array(
        [start_pose[:3, 3], gt_pose[:3, 3], pred_pose[:3, 3]]
    )
